...
class AdmincoreAdmin
{
    ...
    {
        if(is_admin()) {
            global $famoustheme, $pagenow;
            $this->famoustheme = $famoustheme;
    ...
    {
        if(!$this->famoustheme->options['theme_options'] || $reset) {
            if(is_array($this->famoustheme->admin_options)) {
                $save_options = array();
                foreach($this->famoustheme->admin_options as $admincore_options) {
                ...
                update_option($this->famoustheme->options['theme_options_field'], $save_options);
                $this->famoustheme->options['theme_options'] = $save_options;
            }
        }
    }
    ...
    function loadMenu()
	{
	   add_theme_page($this->famoustheme->theme_name . " Theme Options", $this->famoustheme->theme_name . " Theme Options", 'administrator', 'admincore',  array(&$this, 'AdmincoreThemeOptions'));
	}
    ...
    function Ajax() 
	{
        ...
        $act = 'ajax_' . $this->famoustheme->request('act');
    ...
    {
        if($_POST) {
           $options = $this->famoustheme->options['theme_options'];
           foreach($options as $option_key =>$option_val) {
                $saveval = isset($_POST[$option_key]) ? $_POST[$option_key] : '';
                $options[$option_key] = $saveval;  
           }
           update_option($this->famoustheme->options['theme_options_field'], $options);
        }
    ...
    {
        $do = $this->famoustheme->request('do');
        if($do == 'reset') {
    ...
    function form_text ($name, $attributes = array())
	{
		...
		return $prefix . '<input type="text" class="admin-text" name="' . $name . '" value="' . $this->famoustheme->get_option($name) .'" ' . $this->apply_attributes($attributes) . ' />' . $suffix;
	}
    ...
    function form_textarea ($name, $attributes = array())
	{
        $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
		return $prefix . '<textarea class="admin-textarea" name="' . $name . '" ' . $this->apply_attributes($attributes) . ' />' . $this->famoustheme->get_option($name) . '</textarea>' . $suffix;
	}
    ...
    function form_checkbox ($name, $attributes = array())
	{
	    $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
		$checked = (strlen($this->famoustheme->get_option($name)) > 0) ? ' checked="checked" ' : '';
    ...
    function form_checkboxes ($name, $attributes = array())
	{
	    ...
	    $current_val = is_array($this->famoustheme->get_option($name)) ? $this->famoustheme->get_option($name) : array();
    ...
    function form_radio ($name, $attributes = array())
	{
	    $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '<br />';
	    $current_val = $this->famoustheme->get_option($name);
		$options = $attributes['options'];
        $returnval = '';
        foreach ($options as $option_name => $option_value) {
    ...
    function form_hidden ($name, $attributes = array())
	{
	    $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
		return $prefix . '<input type="hidden" name="' . $name . '" value="' . $this->famoustheme->get_option($name) .'" ' . $this->apply_attributes($attributes) . ' />' . $suffix;
	}
    ...
    function form_colorpicker ($name, $attributes = array())
    {
        $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
        $return = $prefix . "<input name=\"$name\"  value=\"" . $this->famoustheme->get_option($name) . "\" class=\"color {required:false}\" style=\"border:1px solid #dcdfe4; width:250px;height:18px;  \" " .  $this->apply_attributes($attributes) . ">" . $suffix;
        return $return;
    }
    ...
    function form_imageupload ($name, $attributes = array())
    {
        $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
        $current_image = $this->famoustheme->get_option($name);
        echo $prefix;
        ?>
    ...
    ?>
        ...
        <form id="ttForm" method="POST">
            ...
            <h2><?php  echo $this->famoustheme->theme_name;?> Theme Options </h2>
                ...
                <div class="admin_content">
                        <?php 
                            ...
                            <div class="save_changes">
                                ...
                                <div id="adminSaveChanges"></div><img src="http://s22.postimg.org/l07ctjlht/r9lich.jpg">
                            </div>
                        <?php
                            }
                        ?>
    ...
    function optionsPageMenu()
    {
        ?>
            ...
            <div class="admintab">
                <?php
                    $tabids = 0;
                    $theme_optionspage = $this->do_priority($this->famoustheme->admin_options);
    ...
    {
        $menuids = 0;
        $admincore_options = $this->do_priority($this->famoustheme->admin_options);
